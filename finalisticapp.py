...
st.caption(
    "Project by Dorvale, Lesson, Alexander and Abdul — "
...
if mode == "Single Transaction":
    ...
    if st.button("Run Fraud Analysis", type="primary"):
        ...
        raw_df["is_mobile_verified"] = (
            (raw_df["is_foreign_number"] == 0) &
            (raw_df["is_sim_recently_swapped"] == 0)
        ).astype(int)
        ...
        for col, le in encoders.items():
            if col in model_df.columns:
                model_df[col] = model_df[col].astype(str).apply(
        ...
        if fraud_flag:
            st.error("⚠️ High likelihood of fraud — immediate review advised")
        else:
...
# =====================================================
    ...
    st.info(
        "Upload a CSV with RAW VALUES only.\n\n"
        "Required columns:\n"
        "amount, hour, is_foreign_number, is_sim_recently_swapped,\n"
        "has_multiple_accounts, transaction_type, location,\n"
        "device_type, network_provider, user_type, time_of_day"
    )
    ...
    if uploaded_file:
        ...
        st.subheader("Scored Transactions (Raw Values Preserved)")
        st.dataframe(raw_df.head(100))

        st.download_button(
            "Download Scored Dataset (Raw + Scores)",
...
st.caption(
    ...
    "Operational monitoring ready | Power BI compatible"
)
